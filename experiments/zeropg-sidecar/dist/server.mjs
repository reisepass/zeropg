...
// experiments/zeropg-sidecar/server.ts
...
function selectStore() {
  ...
  if (useS3) {
    ...
    const bucket2 = process.env.TIGRIS_BUCKET ?? process.env.AWS_BUCKET ?? process.env.S3_BUCKET ?? process.env.R2_BUCKET;
    if (!endpoint || !accessKeyId || !secretAccessKey || !bucket2) {
      throw new Error("S3 backend: missing AWS_ENDPOINT_URL_S3/R2_ENDPOINT or creds/bucket");
    }
    return new R2BlobStore({ endpoint, accessKeyId, secretAccessKey, bucket: bucket2, prefix, region: process.env.AWS_REGION ?? "auto" });
  }
