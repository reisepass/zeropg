...
**3. Durability as a dial.** v0 ships whole snapshots per commit, so strict durability costs seconds on a big database. But "every write durable before ack" is the wrong default for an app that's idle 95% of the day. The serverless-native mode is `sleep`: writes run at memory speed (~150ms round-trip on the demo, most of which is the lease check), and the snapshot uploads *once*, when the platform tells the instance to sleep — plus an idle-flush backstop, because Cloud Run only grants 10 seconds of grace after SIGTERM. The next version ships WAL segments instead of snapshots and makes strict writes ~100–150ms at any size. Same bucket layout, same API.
