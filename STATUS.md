...
Last updated: 2026-06-14 (Track D cold backups merged; capture-frequency knobs added). Raw data in `results/*.jsonl`.
...
- **E6 - cold-backup disaster matrix** (Track D, live against IBM COS): the secondary-backup feature held to the E2b/E4 bar. Each fault x20, byte-identical recovery proven by an md5-per-row checksum table. A: real SIGKILL mid-backup (before object / after object) — primary intact (second blast radius), partial backup ignorable, next backup succeeds + restorable. B: primary CURRENT snapshot deleted → restore from cold, byte-identical. C: FULL primary wipe → rebuild from cold alone, the DB boots and serves SQL. D: aggressive retention (keepLast/maxAgeDays/GFS) never deletes the last restorable backup, and a crash DURING retention GC leaves a restorable set. E: two archivers race the index → no corruption, no lost update, both restorable. F: crash mid-restore → clean retry, byte-identical. G: 1/50/500MB round-trip through the DEFAULT auto-backup wiring + full-disaster restore, byte-identical at each size. H: capture-frequency gate - N rapid compactions with a huge floor produce only 1 cold backup; with ceiling=1ms they produce N. Backups are default-on when a target is configured (`ZeroPGOptions.backup`), no-op otherwise. Raw: `results/e6-disaster.jsonl`.

### Bugs E4/E2c/E6 caught (all fixed, all are now regression probes)
...
12. **THE root cause of 11 (and the lost E2c tails), found by byte-level forensics**: Postgres reads WAL in 8KB pages and a short read at EOF means "end of WAL" — overlay-created segment files ended mid-page, so replay silently stopped at the previous page boundary (the writer's own preallocated 16MB files masked it). A restored 5MB commit lost its final 5KB including the commit record. Fixed: restore extends every touched WAL file (sparse) to full segment size. The recurring `...0C0` flush LSNs = page boundary + end-of-recovery checkpoint record.
13. **Earlier diagnosis of 11, superseded but kept honest** (three violation events, identical signature): `pg_current_wal_flush_lsn()` at a writer's end of life overshoots the last replayable record by exactly 24 bytes (one XLogRecord header). Cross-life LSN chaining is therefore unsound — which is exactly why Litestream rolls a new generation per process restart. Fixed the same way: **WAL ranges never span writer lives**; the first commit of each life compacts (rides the idle flush in sleep mode), `lifeBaseLsn` keeps read-only cold starts at zero uploads, incremental shipping covers the rest of the life. E4 P3's remaining failures were read-routing staleness during the rollout double-instance window (bounded, converges) — the probe now polls to convergence.
14. **Orphaned backup object after a crash mid-backup** (E6 A, the one that would have made backups a lie): `backupOnce` writes the snapshot object (create-if-absent) and then appends to the CAS'd index. A crash in that window left the object written but unreferenced; the retry recomputed the same key, the create-if-absent failed with `PreconditionFailed`, and `adoptExisting` only consulted the index, found nothing, and returned `null` — the good backup was un-adoptable and **nothing was restorable**. Fixed: `adoptExisting` distinguishes "already indexed" (idempotent re-run) from "orphan the index never named", and for the orphan reconstructs the index entry from the manifest being backed up (same key ⇒ same commit point) + a `HEAD` for the true stored size, then appends it. This is exactly what makes E6's `kill-before-object` / `kill-after-object` columns pass.
