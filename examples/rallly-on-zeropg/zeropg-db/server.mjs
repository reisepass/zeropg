...
// citext + pgcrypto contrib extensions a real Prisma app (Rallly) needs. On boot
// it applies any pending migrations IN-PROCESS (idempotent), then serves the
// wire. This is the drop-in replacement for the `postgres:18` service.
...
const wire = await serveWire({
  ...
  port: PORT,
  extensions: { citext, pgcrypto }, // <- what Rallly's schema requires
...
// can't drive single-session PGlite). Idempotent via a marker table.
...
console.log(`[zeropg-db] serving Postgres wire on 0.0.0.0:${PORT} (PGlite + citext + pgcrypto)`)
