...
//   DATABASE_URL=file://./data/shortlink.db   local dev, durable on disk, lock-guarded
...
/** Connect using DATABASE_URL (default file://./data/shortlink.db) and bring the
...
export async function openDb(url?: string): Promise<OpenedDb> {
  const target = url ?? process.env.DATABASE_URL ?? 'file://./data/shortlink.db'
