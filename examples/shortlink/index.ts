// Boot the URL shortener. The only environment knob that matters is DATABASE_URL.
//
//   npx tsx examples/shortlink/index.ts                       # file://./data/shortlink.db
//   DATABASE_URL=memory:// npx tsx examples/shortlink/index.ts # ephemeral
//   DATABASE_URL=https://my-zeropg.run.app npx tsx examples/shortlink/index.ts
//
...
try {
  ...
  console.error(`shortlink boot failed: ${e instanceof Error ? e.message : e}`)
  process.exit(1)
}
...
app.listen(port, () => {
  ...
  console.log(
    `shortlink READY on http://localhost:${bound}  engine=${db.engine}` +
