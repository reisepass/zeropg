...
(Cal.com was demoed and ran, but **dropped from the live set** — its cold start exceeded ~120 s, the heavy Next.js monorepo + on-boot app-store seed make it a poor scale-to-zero showcase. Example code stays in [examples/cloudrun/calcom](examples/cloudrun/calcom) for reference.)
...
**cocoon** is a scale-to-zero [AT Protocol](https://atproto.com) PDS (the server behind a Bluesky account), live at **[pds-scale-to-zero.0rs.org](https://pds-scale-to-zero.0rs.org)**. It's notable because cocoon's driver (Go `pgx`) uses **named server-side prepared statements** - the class that normally collides on the single-session zeropg wire (`42P05`, the sqlx/Diesel wall) - but `pgx` lets you switch to the simple protocol with a **DSN param only** (`?default_query_exec_mode=simple_protocol`), so it runs with no app patch. **How little it took:** the whole thing is a [16-line Dockerfile](examples/cloudrun/pds/Dockerfile) (a thin overlay on the stock `cocoon-pds` image) plus a [service.yaml](examples/cloudrun/pds/service.yaml) that adds the `zeropg-db` sidecar. Public signups are **invite-gated** (PDSs get spammed); see [examples/cloudrun/pds](examples/cloudrun/pds).

**The only change per app** (the app image is the official one, untouched):

1. Replace the `postgres:` service with the **`zeropg-db` sidecar** — the entire "database" is a [22-line Dockerfile](examples/cloudrun/zeropg-db/Dockerfile) + [server.mjs](examples/cloudrun/zeropg-db/server.mjs) (`ZeroPGServer` over a GCS bucket, Postgres wire on `127.0.0.1:5432`): [examples/cloudrun/zeropg-db](examples/cloudrun/zeropg-db) (no migrations) or [zeropg-db-migrate](examples/cloudrun/zeropg-db-migrate) (applies the app's real migrations on first boot + loads contrib extensions).
